...
def _get_interacted_ids_for_user(session, user_id: str):
    ...
    return {
        i.product_id
        for i in session.query(Interaction.product_id).filter(Interaction.user_id == user_id).all()
    }
...
def _get_interacted_ids_for_viewed(session, viewed_ids: list[str]):
    ...
    return {
        i.product_id
        for i in session.query(Interaction.product_id).filter(Interaction.product_id.in_(viewed_ids)).all()
    }
...
class Recommender:
    ...
    def _predict_and_sort(self, user_id: str, candidates: list[str]) -> list:
        """
        Predict rating for each candidates and return a list 
        of(product_id, est_prediction), sorted descending by est_prediction.
        """
        prediction = [self.model.predict(user_id, pid) for pid in candidates]
        sorted_predictions = sorted(prediction, key=lambda x: x.est, reverse = True)
        return sorted_predictions
    
    def recommended_on_user_id(self, user_id: str, skip: int = 0, take: int = 5)-> list[str]:
    ...
    def recommend_on_viewed_ids(self, viewed_ids: list[str], skip: int = 0, take: int = 5) -> list[str]:
        ...
        candidates = [pid for pid in all_product_ids if pid not in interacted_ids]
        sorted_predictions = self._predict_and_sort("annonymous_user", candidates)
        sliced = sorted_predictions[skip: skip+take]
        return  [pred.lid for pred in sliced]
